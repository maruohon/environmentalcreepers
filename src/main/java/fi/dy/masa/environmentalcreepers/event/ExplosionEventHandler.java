...
{
    private final Field field_size;
    private final Field field_causesFire;
    private final Field field_damagesTerrain;

    public ExplosionEventHandler()
    {
        this.field_size             = ReflectionHelper.findField(Explosion.class, "field_77280_f", "size");
        this.field_damagesTerrain   = ReflectionHelper.findField(Explosion.class, "field_82755_b", "damagesTerrain");
        this.field_causesFire       = ReflectionHelper.findField(Explosion.class, "field_77286_a", "causesFire");
    }
    ...
    private void replaceExplosion(ExplosionEvent.Start event, boolean isCreeper)
    {
        ...
        try
        {
            boolean isSmoking = this.field_damagesTerrain.getBoolean(explosion);
            boolean isFlaming = this.field_causesFire.getBoolean(explosion);
            float explosionSize;
            ...
            if (isCreeper && Configs.modifyCreeperExplosionStrength)
            {
                ...
                this.field_size.setFloat(explosion, explosionSize);
            }
            else
            {
                explosionSize = this.field_size.getFloat(explosion);
            }
